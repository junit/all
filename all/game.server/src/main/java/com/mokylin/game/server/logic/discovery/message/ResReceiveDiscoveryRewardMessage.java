...
public class ResReceiveDiscoveryRewardMessage extends Message{
	...
	private Integer msg;
	//物品奖励信息（灵子奖励和物品奖励）
	private List<ItemInfo> baseRewardInfos = new ArrayList<>();
	//物品奖励信息（灵子奖励，金钱奖励（环），物品奖励）
	private List<ItemInfo> treasureRewardInfos = new ArrayList<>();
	...
	public boolean read(ByteBuf buf){
        try {
			...
			for (int i = 0; i < baseRewardInfos_length; i++) {
				baseRewardInfos.add((ItemInfo)readBean(buf, ItemInfo.class));
			}
			//物品奖励信息（灵子奖励，金钱奖励（环），物品奖励）
			int treasureRewardInfos_length = readShort(buf);
			for (int i = 0; i < treasureRewardInfos_length; i++) {
				treasureRewardInfos.add((ItemInfo)readBean(buf, ItemInfo.class));
			}
        } catch (Exception e) {
            log.error(e, e);
            return false;
