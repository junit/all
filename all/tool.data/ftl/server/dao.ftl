...
import com.mokylin.game.server.db.DbFactory;
...
public class ${name?cap_first}Dao {
	...
	public List<${name?cap_first}Bean> select() {
        ...
        try{
			...
			if (interval > 10) {
				com.mokylin.game.server.logger.GlobalLogger.db.error(new StringBuilder().append("${name?cap_first}Dao.").append("select:").append(interval));
			}
            return list;
    	}finally{
			session.close();
    ...
    public void insert(${name?cap_first}Bean bean) {
        ...
        try{
        	...
        	long interval = System.currentTimeMillis() - s;
			if (interval > 10) {
				com.mokylin.game.server.logger.GlobalLogger.db.error(new StringBuilder().append("${name?cap_first}Dao.").append("insert:").append(interval));
			}
    	}finally{
			session.close();
		}
